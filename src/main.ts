...
function showContextMenu(event: MouseEvent, entry: CatalogEntry): void { event.preventDefault(); contextEntry = entry; find<HTMLButtonElement>('#explode-video').hidden = isDirectory(entry) || entry.kind !== 'videos'; const menu = find<HTMLElement>('#context-menu'); menu.style.left = `${event.clientX}px`; menu.style.top = `${event.clientY}px`; menu.hidden = false; }
