...
const state = { files: [] as CatalogEntry[], selected: null as CatalogFile | null, selectedPaths: new Set<string>(), selectionAnchor: '' as string, filter: 'all' as Filter, query: '', sortOrder: 'ascending' as SortOrder, view: 'detailed' as ViewMode, showHidden: false, previewVisible: false, root: '', directory: '', parentListable: false, hasMore: false, page: 0, total: null as number | null, frameTimeRange: null as FrameTimeRange | null, loadingPage: null as number | null, jobs: [] as Job[], screen: 'catalog' as 'catalog' | 'viewer', textContent: '', textDirty: false, catalogDirty: false };
...
async function api<T>(route: string, options?: RequestInit): Promise<T> { const response = await fetch(urlFor(route), options); const body = await response.text(); let payload: T & { error?: string }; try { payload = JSON.parse(body) as T & { error?: string }; } catch { throw new Error(response.ok ? 'The local service returned an invalid response.' : `The local service is unavailable (HTTP ${response.status}).`); } if (!response.ok) throw new Error(payload.error || 'The local service could not complete that request.'); return payload; }
...
async function copyCurrentPath(): Promise<void> {
  const input = find<HTMLInputElement>('#path-input'); const path = input.value.trim(); if (!path) return;
  try {
    if (navigator.clipboard?.writeText) await navigator.clipboard.writeText(path);
    else { input.focus(); input.select(); if (!document.execCommand('copy')) throw new Error('Copy command was unavailable.'); }
...
function visibleFiles(): CatalogEntry[] { return state.files.filter((entry) => isDirectory(entry) || ((state.filter === 'all' || entry.kind === state.filter) && entry.name.toLowerCase().includes(state.query))).sort((a, b) => Number(isDirectory(b)) - Number(isDirectory(a)) || (state.sortOrder === 'ascending' ? a.name.localeCompare(b.name) : b.name.localeCompare(a.name))); }
...
function renderCatalog(): void {
  ...
  const catalogFiles = state.files.filter((entry): entry is CatalogFile => !isDirectory(entry));
  (['all', 'images', 'videos', 'other'] as Filter[]).forEach((filter) => { const count = filter === 'all' ? catalogFiles.length : catalogFiles.filter((file) => file.kind === filter).length; find<HTMLElement>(`#${filter === 'images' ? 'image' : filter === 'videos' ? 'video' : filter}-count`).textContent = String(count); });
  const files = visibleFiles(); const grid = find<HTMLDivElement>('#file-grid'); grid.className = `file-grid view-${state.view}${state.files.length === 0 ? ' hidden' : ''}`; find('#empty-state').classList.toggle('hidden', state.files.length > 0);
  ...
  files.forEach((entry) => {
    ...
    card.addEventListener('click', (event) => selectEntry(entry, event)); card.addEventListener('dblclick', () => directory ? void navigateDirectory(entry.path) : void openViewer(entry)); card.addEventListener('dragstart', (event) => { if (!state.selectedPaths.has(entry.path)) state.selectedPaths = new Set([entry.path]); event.dataTransfer?.setData('application/x-inv-paths', JSON.stringify([...state.selectedPaths])); if (event.dataTransfer) event.dataTransfer.effectAllowed = 'move'; });
    if (directory) { card.addEventListener('dragenter', (event) => { event.preventDefault(); card.classList.add('drop-target'); }); card.addEventListener('dragover', (event) => { event.preventDefault(); if (event.dataTransfer) event.dataTransfer.dropEffect = 'move'; }); card.addEventListener('dragleave', (event) => { if (!card.contains(event.relatedTarget as Node | null)) card.classList.remove('drop-target'); }); card.addEventListener('drop', (event) => { event.preventDefault(); card.classList.remove('drop-target'); let paths: string[] = []; try { const value: unknown = JSON.parse(event.dataTransfer?.getData('application/x-inv-paths') || '[]'); paths = Array.isArray(value) && value.every((item) => typeof item === 'string') ? value : []; } catch { /* Ignore drops from another application. */ } void movePaths(paths, entry.path); }); }
    if (!directory && entry.kind === 'videos') card.addEventListener('contextmenu', (event) => showContextMenu(event, entry)); grid.append(card);
  });
}
...
async function saveText(file: CatalogFile, editor: HTMLTextAreaElement): Promise<void> { const button = find<HTMLButtonElement>('#save-text'); button.disabled = true; try { await api('/api/text', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ path: file.path, content: editor.value }) }); state.textDirty = false; find('#text-status').textContent = 'Saved'; await refreshCatalog(); } catch (error) { toast(error instanceof Error ? error.message : String(error)); } finally { button.disabled = false; } }
...
function catalogRoute(): string { return `/api/catalog?path=${encodeURIComponent(state.directory)}&hidden=${state.showHidden ? '1' : '0'}&query=${encodeURIComponent(state.query)}&filter=${state.filter}&offset=${state.page * 250}`; }
...
async function openFolder(path: string): Promise<void> { state.page = 0; const result = await api<CatalogResponse>('/api/open', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ path, showHidden: state.showHidden, query: state.query, filter: state.filter }) }); state.root = result.root; state.selected = null; state.selectedPaths.clear(); state.selectionAnchor = ''; state.catalogDirty = false; applyCatalog(result); closeViewer(); subscribeToDirectory(); }
...
async function movePaths(paths: string[], destination: string): Promise<void> { if (!paths.length) return; try { await api('/api/files/move', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ paths, destination }) }); state.selectedPaths.clear(); state.selectionAnchor = ''; toast(`${paths.length} item${paths.length === 1 ? '' : 's'} moved.`); await refreshCatalog(); } catch (error) { toast(error instanceof Error ? error.message : String(error)); } }
...
let contextFile: CatalogFile | null = null;
function showContextMenu(event: MouseEvent, file: CatalogFile): void { event.preventDefault(); contextFile = file; const menu = find<HTMLElement>('#context-menu'); menu.style.left = `${event.clientX}px`; menu.style.top = `${event.clientY}px`; menu.hidden = false; }
function hideContextMenu(): void { find<HTMLElement>('#context-menu').hidden = true; contextFile = null; }
...
find<HTMLButtonElement>('#up-button').addEventListener('click', () => { if (!state.parentListable) return; if (state.directory) { const separator = state.directory.lastIndexOf('/'); void navigateDirectory(separator === -1 ? '' : state.directory.slice(0, separator)); } else { const separator = state.root.lastIndexOf('/'); void openFolder(separator > 0 ? state.root.slice(0, separator) : '/'); } });
...
app.querySelectorAll<HTMLButtonElement>('.nav-item').forEach((button) => button.addEventListener('click', () => { state.filter = button.dataset.filter as Filter; state.page = 0; app.querySelectorAll('.nav-item').forEach((item) => item.classList.toggle('active', item === button)); find('#location-name').textContent = state.directory || 'Root'; void refreshCatalog(); }));
...
find('#explode-video').addEventListener('click', () => { const file = contextFile; hideContextMenu(); if (!file) return; void api<Job>('/api/videos/explode', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ path: file.path }) }).then(applyJobUpdate).catch((error) => toast(error instanceof Error ? error.message : String(error))); }); document.addEventListener('click', hideContextMenu);
